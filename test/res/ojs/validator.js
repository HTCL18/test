...
function noComma(obj) {
	var objindexC = obj.indexOf("，");
	var objindexE = obj.indexOf(",");
	if(objindexC != -1|| objindexE != -1){
		return false;
    }else {
        return true;
    }
    ...
    $(obj).find("[datacol=yes]").each(function () {
        if ($(this).attr("checkexpession") != undefined) {
            ...
            switch ($(this).attr("checkexpession")) {
                ...
                {
                    if (!noComma(value)) {
                        Validatemsg = $(this).attr("err") + "不能包含逗号！\n";
                        Validateflag = false;
                        tipCss($(this), Validatemsg); return false;
                    }else{
                ...
                case "Num":
                    {
                        if (!isInteger(value)) {
                            Validatemsg = $(this).attr("err") + "必须为0或非0开头的数字！\n";
                            Validateflag = false;
                            tipCss($(this), Validatemsg); return false;
                        }else{
                ...
                {
                   if (!isInteger(value)) {
                        Validatemsg = $(this).attr("err") + "必须为" + $(this).attr("min") + "~" + $(this).attr("max") + "间的数字！\n";
                        Validateflag = false;
                        tipCss($(this), Validatemsg); return false;
                    }else if($(this).attr("min") > parseInt(value) || parseInt(value) > $(this).attr("max")){
                    	Validatemsg = $(this).attr("err") + "必须为" + $(this).attr("min") + "~" + $(this).attr("max") + "间的数字！\n";
                        Validateflag = false;
                        tipCss($(this), Validatemsg); return false;
                    }
                ...
                case "LenNum":
                    {
                        if (!isLenNum(value, $(this).attr("length"))) {
                            Validatemsg = $(this).attr("err") + "必须小于等于" + $(this).attr("length") + "位数字！\n";
                            Validateflag = false;
                            tipCss($(this), Validatemsg); return false;
                        }
                        break;
                ...
                case "LenNum2":
                {
                	  if (!isIntegerOrNull(value)) {
                          ...
                          Validateflag = false;
                          tipCss($(this), Validatemsg); return false;
                      }else if (!isLenNumOrNull(value, $(this).attr("length"))) {
                        Validatemsg = $(this).attr("err") + "必须小于等于" + $(this).attr("length") + "位数字！\n";
                        Validateflag = false;
                        tipCss($(this), Validatemsg); return false;
                    }
